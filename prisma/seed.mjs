...
async function main() {
  ...
  const user = await prisma.user.upsert({
    ...
    update: {},
    create: { name: "Demo User", email: "demo@re2.ai", hashedPassword, role: "admin" },
  });
  ...
  const list1 = await prisma.list.create({ data: { name: "SF Restaurants", description: "Top restaurants in San Francisco for Q1 outreach", userId: user.id } });
  const list2 = await prisma.list.create({ data: { name: "High Score Prospects", description: "Businesses with RE2 score above 85", userId: user.id } });
  ...
  for (const biz of sfRestaurants) {
    await prisma.listItem.create({ data: { listId: list1.id, businessId: biz.id } });
  }
  const highScoreBiz = allBusinesses.filter(b => b.re2Score >= 85);
  for (const biz of highScoreBiz) {
    await prisma.listItem.create({ data: { listId: list2.id, businessId: biz.id } });
  }

  const campaign1 = await prisma.campaign.create({
    data: { name: "Q1 Restaurant Outreach", subject: "Partnership opportunity for your restaurant", body: "Hi,\n\nI came across your restaurant and was impressed...", status: "active", listId: list1.id, userId: user.id, totalSent: 45, totalOpened: 32, totalReplied: 12, totalBounced: 2 },
  });
  await prisma.campaign.create({
    data: { name: "High-Value Prospect Intro", subject: "Exclusive offer for top-rated businesses", body: "Dear team,\n\nYour business stands out...", status: "draft", listId: list2.id, userId: user.id, totalSent: 0, totalOpened: 0, totalReplied: 0, totalBounced: 0 },
  });
  console.log("Created sample campaigns");

  for (const biz of sfRestaurants) {
    await prisma.outreachEmail.create({
      data: { campaignId: campaign1.id, businessId: biz.id, userId: user.id, channel: "email", subject: "Partnership opportunity", body: "Hi, I'd love to discuss a partnership...", status: "sent", sentAt: new Date(Date.now() - Math.random() * 7 * 86400000) },
    });
  }
  ...
  for (const deal of dealData) { await prisma.deal.create({ data: { ...deal, userId: user.id } }); }
  ...
  const activityData = [
    ...
    { type: "deal_stage_changed", title: "Deal moved to Negotiation", businessId: allBusinesses[1].id },
  ...
  for (const a of activityData) { await prisma.activity.create({ data: { ...a, userId: user.id } }); }
